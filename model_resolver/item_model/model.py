...
from typing import Optional, Literal, ClassVar, Generator
...
class ItemModelBase(BaseModel):
    ...
    def resolve(self, ctx: Context, vanilla: Vanilla, item: Item) -> Generator["ItemModelModel", None, None]:
        yield from []
...
class ItemModelConditionBase(ItemModelBase):
    ...
    property: Literal["minecraft:using_item", "minecraft:broken", "minecraft:damaged", "minecraft:has_component", "minecraft:fishing_rod/cast", "minecraft:bundle/has_selected_item", "minecraft:xmas", "minecraft:selected", "minecraft:carried", "minecraft:shift_down", "minecraft:custom_model_data"]
    on_true: "ItemModelAll"
    on_false: "ItemModelAll"

    def resolve_condition(self, ctx: Context, vanilla: Vanilla, item: Item) -> bool: raise NotImplementedError
...
class ItemModelConditionBroken(ItemModelConditionBase):
    ...
    def resolve_condition(self, ctx: Context, vanilla: Vanilla, item: Item) -> bool:
        ...
        if not "minecraft:max_damage" in item.components:
            return False
        remaining = item.components["minecraft:max_damage"] - item.components["minecraft:damage"]
        return remaining <= 1
...
type ItemModelCondition = ItemModelConditionUsingItem | ItemModelConditionBroken | ItemModelConditionDamaged | ItemModelConditionHasComponent | ItemModelConditionFishingRodCast | ItemModelConditionBundleHasSelectedItem | ItemModelConditionXmas | ItemModelConditionSelected | ItemModelConditionCarried | ItemModelConditionShiftDown | ItemModelConditionCustomModelData
...
class ItemModelSelectBase(ItemModelBase):
    ...
    property: Literal["minecraft:main_hand", "minecraft:charge_type", "minecraft:trim_material", "minecraft:block_state", "minecraft:display_context", "minecraft:custom_model_data"]
    cases: list[SelectCase] = Field(default_factory=list)
    fallback: "ItemModelAll"
    possible_values: ClassVar[list[str]] = []

    def resolve_select(self, ctx: Context, vanilla: Vanilla, item: Item) -> "ItemModelAll":
        # Not possible to implement
...
class ItemModelSelectBlockState(ItemModelSelectBase):
    ...
    def resolve_select(self, ctx: Context, vanilla: Vanilla, item: Item) -> "ItemModelAll":
        ...
        if not self.block_state_property in item.components["minecraft:block_state"]:
            return self.fallback
        return self.resolve_case(item.components["minecraft:block_state"][self.block_state_property])
    
class ItemModelSelectDisplayContext(ItemModelSelectBase):
    property: Literal["minecraft:display_context"]
    possible_values : ClassVar[list[str]] = ["none", "thirdperson_lefthand", "thirdperson_righthand", "firstperson_lefthand", "firstperson_righthand", "gui", "head", "ground", "fixed"]
...
class ItemModelSelectCustomModelData(ItemModelSelectBase):
    ...
    def resolve_select(self, ctx: Context, vanilla: Vanilla, item: Item) -> "ItemModelAll":
        ...
        if index >= len(item.components["minecraft:custom_model_data"]["strings"]):
            return self.fallback
        return self.resolve_case(item.components["minecraft:custom_model_data"]["strings"][index])
            
    
type ItemModelSelect = ItemModelSelectMainHand | ItemModelSelectChargeType | ItemModelSelectTrimMaterial | ItemModelSelectBlockState | ItemModelSelectDisplayContext | ItemModelSelectCustomModelData
...
class ItemModelRangeDispatchBase(ItemModelBase):
    """
    minecraft:custom_model_data, minecraft:bundle/fullness, minecraft:damage, minecraft:count, minecraft:cooldown, minecraft:time, minecraft:compass, minecraft:crossbow/pull, minecraft:use_duration or minecraft:use_cycle
    """
    type: str = "minecraft:range_dispatch"
    property: Literal["minecraft:custom_model_data", "minecraft:bundle/fullness", "minecraft:damage", "minecraft:count", "minecraft:cooldown", "minecraft:time", "minecraft:compass", "minecraft:crossbow/pull", "minecraft:use_duration", "minecraft:use_cycle"]
    scale: Optional[float] = 1.0
    entries: list[RangeDispatchEntry]
    fallback: "ItemModelAll"

    def resolve_range_dispatch(self, ctx: Context, vanilla: Vanilla, item: Item) -> float:
        return 0.0
