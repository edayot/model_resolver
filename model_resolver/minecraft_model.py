...
type MultiTexture = tuple[str | Image.Image, TintSource | None]

class TextureDict(BaseModel):
    sprite: str
    force_translucent: bool = False

type TextureSource = None | str | Image.Image | TextureDict | tuple[MultiTexture, ...]


class Texture(RootModel[TextureSource]):
    root: TextureSource

    def resolve(self, getter: PackGetter) -> Image.Image: 
        if isinstance(self.root, Image.Image):
            return self.root
        if isinstance(self.root, str):
            return getter.assets.textures[self.root].image
        if isinstance(self.root, TextureDict):
            return getter.assets.textures[self.root.sprite].image
        if self.root is None:
            with open(Path(__file__).parent / "missingno.png", "rb") as f:
                return Image.open(f).convert("RGBA")
        raise ValueError(f"Unsupported texture source: {self.root}")
