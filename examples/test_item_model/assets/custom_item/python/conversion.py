...
pixels = [
    [
        [{"active": False, "color": "#000000"} for _ in range(grid_size)]
...
for element in model["elements"]:
    ...
    x_min, y_min, z_min = (
        min(from_coords[0], to_coords[0]),
        min(from_coords[1], to_coords[1]),
        min(from_coords[2], to_coords[2]),
    )
    x_max, y_max, z_max = (
        max(from_coords[0], to_coords[0]),
        max(from_coords[1], to_coords[1]),
        max(from_coords[2], to_coords[2]),
    )
