...
class PixelGrid:
    def __init__(self, master):
        ...
        self.pixels = [
            [
                [{"active": False, "color": "#000000"} for _ in range(self.grid_size)]
    ...
    def create_widgets(self):
        ...
        self.save_button = tk.Button(
            self.control_frame, text="Save", command=self.save_grid
        )
        self.save_button.pack(pady=10)

        self.load_button = tk.Button(
            self.control_frame, text="Load", command=self.load_grid
        )
        self.load_button.pack(pady=10)

        self.generate_button = tk.Button(
            self.control_frame, text="Generate Command", command=self.generate_command
        )
        self.generate_button.pack(pady=10)

        self.color_button = tk.Button(
            self.control_frame, text="Choose Color", command=self.choose_color
        )
        self.color_button.pack(pady=10)

        self.layer_label = tk.Label(
            self.control_frame, text=f"Layer: {self.current_layer}"
        )
        self.layer_label.pack(pady=10)

        self.prev_layer_button = tk.Button(
            self.control_frame, text="Previous Layer", command=self.prev_layer
        )
        self.prev_layer_button.pack(pady=10)

        self.next_layer_button = tk.Button(
            self.control_frame, text="Next Layer", command=self.next_layer
        )
    ...
    def on_canvas_click(self, event):
        ...
        if 0 <= x < self.grid_size and 0 <= y < self.grid_size:
            ...
            self.pixels[x][y][z]["color"] = (
                self.current_color if self.pixels[x][y][z]["active"] else "#000000"
            )
            self.draw_grid()
    ...
    def generate_command(self):
        ...
        close_button = tk.Button(
            command_window, text="Close", command=command_window.destroy
        )
